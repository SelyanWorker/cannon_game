...
void main()
{
	if ( gl_InvocationID == 0 )
	{
		...
		float tessLevel = maxTessLevel * k;
...
void main()
{
	...
	vec4 addColor;
	if(length(input.position - interPoint) <= brushSize) addColor = vec4(brushColor, 0);
	else addColor = vec4(0);
	
	//outColor = vec4(normalize(interPoint), 1);
	//outColor = vec4(s, 1);
	outColor = resultLightColor + addColor;
	//outColor = vec4(addColor.xyz, 1);
	//outColor = vec4(1, 0, 0, 1);
	//outColor = vec4(input.normal, 1);
